...
class GANSegAgent(BaseAgent):
    ...
    def __init__(self, config):
        ...
        self.optimG = torch.optim.Adam(self.netG.parameters(), lr=self.config.learning_rate, betas=self.config.betas)
        self.optimD = torch.optim.Adam(self.netD.parameters(), lr=self.config.learning_rate, betas=self.config.betas)
    ...
    def train_one_epoch(self):
        ...
        for x, y in tqdm_batch:
            ...
            loss_D_real = self.disc_bce_loss(D_real_out, disc_label)
            ...
            y_fake_combined = x*y_fake
            D_fake_out = self.netD(y_fake_combined.detach())

            loss_D_fake = self.disc_bce_loss(D_fake_out, disc_label)
...
#             loss_D.backward()
            ...
            self.netG.zero_grad()
            disc_label.fill_(self.real_label)
            D_out = self.netD(y_fake_combined)
            adv_loss = self.disc_bce_loss(D_out, disc_label)
            ce_loss = self.loss_ce(pred, y)
            dice_loss = self.loss_dice(pred, y)

            loss_G = 1.2*ce_loss + dice_loss + 0.01*adv_loss
            loss_G.backward()
            self.optimG.step()
