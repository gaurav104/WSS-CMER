...
from datasets.brats import BratsLoader, ClfAugPipeline#,All, HFlip, Rotate, Translate, Scale # importing data Loader
...
from medpy.metric.binary import dc, assd , hd, ravd
# from utils.misc import print_cuda_statistics

from utils.CAM_utils import max_norm, save_cam
from utils.gradcam import CAM
...
#######################################################################################################################################
		...
		# mean_hd_distance_3d = {sal_map : {x : np.mean(np.array(hd_distance_3d[sal_map][x])) for x in thresholds} for sal_map in ['cam', 'grad_cam', 'grad_cam_pp']}
		# hd_dest = os.path.join(self.config.project_directory, self.config.exp_name, "{}_hd_3d.json".format(data))
		# with open(hd_dest, 'w') as fp:
		# 	json.dump(mean_hd_distance_3d, fp)

		# mean_avd_3d = {sal_map : {x : np.mean(np.array(avd_3d[sal_map][x])) for x in thresholds} for sal_map in ['cam', 'grad_cam', 'grad_cam_pp']}
		# avd_dest = os.path.join(self.config.project_directory, self.config.exp_name, "{}_avd_3d.json".format(data))
		# with open(avd_dest, 'w') as fp:
		# 	json.dump(mean_avd_3d, fp)
