...
def process_config(json_file):
	...
	# 	config.project_directory = "/home/gp104/projects/def-josedolz/gp104/project_wss_brats/project_wss"
	# 	config.summary_dir = os.path.join(config.project_directory, config.exp_name, "summaries/")
	# 	config.checkpoint_dir = os.path.join(config.project_directory, config.exp_name, "checkpoints/")
	# 	config.out_dir = os.path.join(config.project_directory, config.exp_name, "out/")
	# 	config.log_dir = os.path.join(config.project_directory, config.exp_name, "logs/")
	# 	create_dirs([config.summary_dir, config.checkpoint_dir, config.out_dir, config.log_dir])
	# 	config_dest = os.path.join(config.project_directory, config.exp_name, "config.json")
		
		# with open(config_dest, 'w') as fp:
		# 	json.dump(config, fp)
			
	# else:
	config.project_directory = "../experiments"
	config.summary_dir = os.path.join(config.project_directory, config.exp_name, "summaries/")
	config.checkpoint_dir = os.path.join(config.project_directory, config.exp_name, "checkpoints/")
	config.out_dir = os.path.join(config.project_directory, config.exp_name, "out/")
	config.log_dir = os.path.join(config.project_directory, config.exp_name, "logs/")
